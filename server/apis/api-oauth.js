...
passport.deserializeUser((id, done) => {
	...
	UsersController.read({ id: id })
		.then(result => {
			if (result.rowCount === 0) return done(null, false);

			log.debug(
				'🔑  deserializeUser success user.id: ${result.rows[0].id}, user.username: ${result.rows[0].username}',
			);
...
server.deserializeClient((id, done) => {
	...
	ClientsController._read({ id: id })
		.then(result => {
			...
			log.debug(
				'🔑  deserializeClient success client.id: ${id}, result.rows[0].name: ${result.rows[0].name}',
			);
			return done(null, result.rows[0]);
		})
		.catch(err => {
...
async function createToken(data, tokenType) {
	try {
		...
		log.debug(
			`🔑  generateTokens -> Generated new tokens for client.id: ${data.clientId} and user.id: ${
				data.userId
			} =tokens: ${tokens}`,
		);
		return tokens;
...
server.grant(
	oauth2orize.grant.code((client, redirectUri, user, ares, done) => {
		log.debug(
			`🔑  grant.code atempt for client: ${client.id}=${client.name} and user: ${user.id}=${
				user.username
			}`,
		);
		...
		GrantCodesController._create(
			...
			.then(result => {
				...
				log.debug(
					`🔑  grant.code.value: ${codeValue} success for client: ${client.id}=${
						client.name
					} and user: ${user.id}=${user.username}`,
				);
				done(null, codeValue);
			})
			.catch(err => {
...
server.grant(
	oauth2orize.grant.token((client, user, ares, done) => {
		log.debug(
			`🔑  grant.token atempt for client: ${client.id}=${client.name} and user: ${user.id}=${
				user.username
			}`,
		);
		...
		createToken(
			...
			.then(tokens => {
				log.debug(
					`🔑  grant.token.value: ${tokens[0].value} success for client: ${client.id}=${
						client.name
					} and user: ${user.id}=${user.username}`,
				);
	...
	oauth2orize.exchange.code((client, code, redirectUri, done) => {
		...
		GrantCodesController._read({ value: code, client_id: client.id })
			.then(result => {
				...
				log.debug(
					`🔑  exchange.code found for client: ${client.id}=${client.name} and code: ${code}`,
				);
			...
			.then(
				result => {
					createToken(
						...
						.then(tokens => {
							log.debug(
								`🔑  exchange.code token: ${tokens} created for client: ${client.id}=${
									client.name
								} and code: ${code}`,
							);
...
server.exchange(
	oauth2orize.exchange.password((client, username, password, scope, done) => {
		log.debug(
			`🔑  exchange.password for client: ${client.id}, user: ${username} and password: ${password}`,
		);

		UsersController.read({ email: username })
			.then(result => {
				if (result.rowCount === 0) {
					done(
						new AuthError(
							`A user with this username ${username} does not exist`,
			...
			.then(
				tokens => {
					log.debug(
						`🔑  exchange.password token created: ${tokens} created for ${
							client.id
...
server.exchange(
	oauth2orize.exchange.clientCredentials((client, scope, done) => {
		...
		createToken(
			...
			.then(tokens => {
				log.debug(
					`🔑  exchange.clientCredentials token: ${tokens} created for client: ${client.id}=${
						client.name
					}`,
				);
	...
	oauth2orize.exchange.refreshToken((client, refreshToken, scope, done) => {
		...
		RefreshTokenModel.findOne({ value: refreshToken })
			...
			.then(
				async () => {
					try {
						const atm = await AccessTokenModel.findOneAndRemove({
							userId: client.userId,
							clientId: client.id,
						});
						const rtm = await RefreshTokenModel.findOneAndRemove({
							userId: client.userId,
							clientId: client.id,
						});

						log.debug(
							`🔑  exchange.refreshToken tokens deleted for client: ${client.id}=${client.name}`,
						);
			...
			.then(
				tokens => {
					log.debug(
						`🔑  exchange.refreshToken token: ${tokens} created for client: ${client.id}=${
							client.name
						}`,
					);
	...
	UsersController.read({ email: username })
		...
		.then(
			result => {
				...
				const hashedpassword = crypto
					.pbkdf2Sync(password, client.salt, 10000, 512, 'sha512')
					.toString('hex');
...
const cbClientPasswordStrategy = (clientId, clientSecret, done) => {
	log.debug(
		`🔑  cbClientPasswordStrategy for clientId: ${clientId} and clientSecret: ${clientSecret}`,
	);
	...
	ClientsController._read({ id: clientId })
		.then(result => {
			...
			log.debug(
				`🔑  in cbClientPasswordStrategy for clientId: ${clientId} and clientSecret: ${clientSecret} -> done`,
			);
			return done(null, client);
		})
		.catch(err => {
	...
	AccessTokenModel.findOne({ value: bearerToken }, (err, accessToken) => {
		...
		if (!accessToken) {
			RefreshTokenModel.findOne({ value: bearerToken }, (err, refreshToken) => {
				if (err) return done(new AuthError(err));
				if (!refreshToken)
					return done(new AuthError(`A token does not exist in db`, 'invalid_verification'));
...
const cbEmailBearerStrategy = (bearerToken, done) => {
	...
	UsersController.read({ verify_token: bearerToken })
		.then(result => {
			if (result.rowCount === 0)
				return done(new AuthError(`A token does not exist in db`, 'invalid_verification'));
...
const cbJWTStrategy = (req, jwtPayload, done) => {
	...
	if (jwtPayload.verify_expired) {
		UsersController.read({ id: jwtPayload.userId })
			.then(result => {
				if (result.rowCount === 0)
					return done(
						new AuthError(`User with this id (${jwtPayload.userId}) not exists`, 'invalid_data'),
					);
				...
				if (!user.verified) {
					if (user.verify_expired > new Date()) {
						...
						log.debug(
							`🔑  cbJWTStrategy for NOT verified userId: ${jwtPayload.userId} - token is OK`,
						);
						return done(null, jwtPayload);
					} else {
						// log out user

						UsersController.update({ id: jwtPayload.userId }, { loged: false })
							.then(() => {
								req.logout();

								log.debug(
									`🔑  cbJWTStrategy for NOT verified userId: ${
										jwtPayload.userId
									} - session expired`,
								);
					...
					createToken(data, 'jwt')
						.then(tokens => {
							...
							log.debug(
								`🔑  cbJWTStrategy for verified userId: ${jwtPayload.userId} - token refreshed`,
							);
		...
		if (BlackListCache.get(token)) {
			...
			log.debug(
				`🔑  cbJWTStrategy for verified userId: ${
					jwtPayload.userId
				} - token in black list. Auth rejected`,
			);
		...
		if (expirationTime <= 0) {
			...
			expirationTime <=
			config.security.jwtTokenExpires * 1000 * config.security.boundaryBeginExpires
		) {
			const data = {
				userId: jwtPayload.userId,
				clientId: jwtPayload.clientId,
			...
			createToken(data, 'jwt')
				.then(tokens => {
					...
					log.debug(
						`🔑  cbJWTStrategy for verified userId: ${jwtPayload.userId} - token is refreshed`,
					);
					return done(null, tokens[0].value, params);
				})
				.catch(err => {
...
const cbVerifyMailToken = (req, res, done) => {
	...
	UsersController.read({ verify_token: req.query.token, verify_expired: { $gt: new Date() } })
		.then(result => {
			if (result.rowCount === 0) {
			...
			return UsersController.update({ id: result.rows[0].id }, { verified: true }, true)
				.then(result => {
					const user = result.rows[0];
...
const cbRegistrationStrategy = function(req, res, done) {
	...
	try {
		...
		if (!req.body.username)
			throw new AuthError('Missing required parameter: username', 'invalid_request');
		if (!req.body.password)
			throw new AuthError('Missing required parameter: password', 'invalid_request');
		if (!req.body.client_name)
			throw new AuthError('Missing required parameter: client_name', 'invalid_request');
		if (!req.body.scope)
			throw new AuthError('Missing required parameter: scope', 'invalid_request');
		if (!req.body.email)
			throw new AuthError('Missing required parameter: e-mail', 'invalid_request');
		if (!utils.validateEmail(req.body.email))
			throw new AuthError('Wrong parameter: e-mail', 'invalid_data');

		return UsersController.read({ email: req.body.email })
			.then(result => {
				if (result.rowCount) {
					return Promise.reject(
						`User with this email (${req.body.email}) already exists`,
						'invalid_data',
					);
				}
		...
		function onSaveUser(resultUser) {
			...
			return ClientsController._create(client, true).then(resultClient => {
...
const cbResetPassword = (req, res, done) => {
	try {
		...
		if (req.method === 'POST') {
			if (!req.body.password)
				throw new AuthError('Missing body param: password', 'invalid_request');
			...
			onSaveUser = result => {
				...
				if (config.security.sendEmailVerification) {
					const emailData = {
						...
						context: {
							url: `http://${config.host}:${config.port}/api/oauth2/verifytoken?token=${
								updatedUser.verify_token
							}&token_type=reset`,
		...
		UsersController.read(findQuery)
			.then(result => {
				...
				return UsersController.update({ id: user.id }, user, true);
			})
			.then(onSaveUser, rej => {})
			.catch(err => {
...
for (let index = 0; index < config.security.securityStrategy.length; index++) {
	...
	if (element === 'client-basic')
		passport.use(
			'client-basic',
			new BasicStrategy({ passReqToCallback: true }, cbClientBasicStrategy),
		);
	if (element === 'oauth2-client-password')
...
const isSecurityAuthenticated = passport.authenticate(config.security.securityStrategy, {
	session: false,
});
...
const codeDecision = [server.decision()];
...
router.post('/resetpassword', isEmailAuthenticated, [
	cbResetPassword,
	server.token(),
	server.errorHandler(),
]);
