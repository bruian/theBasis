...
router.get('/main-user', (req, res) => {
	...
	UserController.getUser(condition)
		.then(data => {
			...
			return res.json({ data, packet: condition.packet });
		})
		.catch(err => {
			log.warn(
				`/main-user:get |-> name:${err.name} | status:${err.jse_info.status} | message:${
					err.message
				}`,
			);
...
router.get('/users/:id', (req, res) => {
	...
	UserController.getUsers(condition, (err, data) => {
		...
		log.debug(
			`/users:return |-> like: ${condition.like} | offset: ${condition.offset} | partid: ${
				req.headers.partid
			}`,
		);
		return res.json({ data, partid: req.headers.partid });
	});
});
...
router.get('/groups/:id', (req, res) => {
	...
	GroupController.getGroups(condition, (err, data) => {
		...
		log.debug(
			`/groups:return |-> like: ${condition.like} | offset: ${condition.offset} | partid: ${
				req.headers.partid
			}`,
		);
		return res.json({ data, partid: req.headers.partid });
...
router.get('/groups', (req, res) => {
	...
	GroupController.getGroups(condition, (err, data) => {
		...
		return res.json({ data, partid: req.headers.partid, packet: condition.packet });
	});
});
...
router.get('/contexts', (req, res) => {
	...
	ContextController.getContexts(condition, (err, data) => {
		...
		log.debug(
			`/contexts:get |-> task_id:${condition.task_id} | offset:${condition.offset} | limit:${
				condition.limit
			} | for user:${condition.mainUser_id} | elements:[${ids}]`,
		);
...
router.post('/contexts', (req, res) => {
	...
	ContextController.addContext(condition, (err, data) => {
		...
		log.debug(
			`/contexts:post |-> task_id:${condition.task_id} | for user: ${condition.mainUser_id}`,
		);
...
router.delete('/contexts', (req, res) => {
	...
	ContextController.deleteContext(condition, (err, data) => {
		...
		log.debug(
			`/context:delete |-> task_id:${condition.task_id} | for user: ${condition.mainUser_id}`,
		);
...
router.get('/tasks', (req, res) => {
	...
	TaskController.getTasks(condition)
		.then(data => {
			...
			log.debug(
				`/tasks:get ${Date.now() - timeStart}ms |-> like:${condition.searchText} | group_id:${
					condition.group_id
				} | parent_id:${condition.parent_id} | for user:${condition.mainUser_id} | offset:${
					condition.offset
				} | partid:${req.headers.partid} | elements:[${ids}]`,
			);
		...
		.catch(err => {
			log.warn(
				`/task:get |-> name:${err.name} | status:${err.jse_info.status} | message:	${err.message}`,
			);
...
router.post('/tasks', (req, res) => {
	...
	const onTaskCreated = taskData => {
		log.debug(
			`/tasks:post ${Date.now() - timeStart}ms |-> id:${taskData[0].task_id} | parent:${
				taskData[0].parent
		...
		return ActivityController.createActivity(condition)
			.then(activityData => {
				log.debug(
					`/activity:post ${Date.now() - timeStart}ms |-> id:${condition.task_id} | group:${
						condition.group_id
	...
	TaskController.createTask(condition)
		.then(onTaskCreated)
		.catch(err => {
			log.warn(
				`/tasks:post |-> name:${err.name} | status:${err.jse_info.status}	| message:${err.message}`,
			);
...
router.put('/tasks', (req, res) => {
	...
	TaskController.updateTask(condition, values)
		.then(taskData => {
			log.debug(
				`/tasks:put ${Date.now() - timeStart}ms |-> id:${taskData.task_id} | for user:${
		...
		.catch(err => {
			log.warn(
				`/task:put |-> name:${err.name} | status:${err.jse_info.status} | message:${err.message}`,
			);
...
router.put('/tasks/order', (req, res) => {
	...
	const onTaskUpdated = taskData => {
		log.debug(
			`/tasks/order ${Date.now() - timeStart}ms |-> id:${condition.task_id} | parent:${
				condition.parent_id
			} | group:${condition.group_id} | position:${condition.position} | isBefore:${
				condition.isBefore
	...
	TaskController.updatePosition(condition)
		.then(onTaskUpdated)
		.catch(err => {
			log.warn(
				`/tasks/order:put |-> name:${err.name} | status:${err.jse_info.status} | message:${
					err.message
				}`,
			);
...
router.get('/activity', (req, res) => {
	...
	ActivityController.getActivity(condition)
		.then(data => {
			const ids = data.map(el => el.id).toString();
			log.debug(
				`/activity:get ${Date.now() - timeStart}ms |-> like:${condition.searchText} | group_id:${
					condition.group_id
		...
		.catch(err => {
			log.warn(
				`/activity:get |-> name:${err.name} | status:${err.jse_info.status} | message:${
					err.message
				}`,
			);
...
router.post('/activity', (req, res) => {
	...
	ActivityController.createActivity(condition)
		...
		.catch(err => {
			log.warn(
				`/activity:post |-> name:${err.name} | status:${err.jse_info.status} | message:${
					err.message
				}`,
			);
...
router.put('/activity', (req, res) => {
	...
	ActivityController.updateActivity(condition, values)
		.then(data => {
			log.debug(
				`/activity:put ${Date.now() - timeStart}ms |-> id:${condition.id} | for user:${
					condition.mainUser_id
				}`,
			);
		...
		.catch(err => {
			log.warn(
				`/activity:put |-> name:${err.name} | status:${err.jse_info.status} | message:${
					err.message
				}`,
			);
...
router.get('/sheets', (req, res) => {
	...
	SheetController.getSheets(condition)
		.then(data => {
			const ids = data.map(el => el.id).toString();
			log.debug(
				`/sheets:get ${Date.now() - timeStart}ms |-> | for user:${
					condition.mainUser_id
				} | elements:[${ids}]`,
			);
		...
		.catch(err => {
			log.warn(
				`/sheets:get |-> name:${err.name} | status:${err.jse_info.status} | message:${err.message}`,
			);
...
router.post('/sheets', (req, res) => {
	...
	SheetController.createSheet(condition, values)
		...
		.catch(err => {
			log.warn(
				`/sheets:post |-> name:${err.name} | status:${err.jse_info.status} | message:${
					err.message
				}`,
			);
...
router.put('/sheets', (req, res) => {
	...
	SheetController.updateSheet(condition, values)
		.then(data => {
			log.debug(
				`/sheets:put ${Date.now() - timeStart}ms |-> id:${data.id} | for user:${
					condition.mainUser_id
				}`,
			);
		...
		.catch(err => {
			log.warn(
				`/sheets:put |-> name:${err.name} | status:${err.jse_info.status} | message:${err.message}`,
			);
...
router.delete('/sheets', (req, res) => {
	...
	SheetController.deleteSheet(condition)
		.then(sheetData => {
			log.debug(
				`/sheets:delete ${Date.now() - timeStart}ms |-> id:${condition.id} | for user:${
					condition.mainUser_id
				}`,
			);
		...
		.catch(err => {
			log.warn(
				`/sheets:delete |-> name:${err.name} | status:${err.jse_info.status} | message:${
					err.message
				}`,
			);
