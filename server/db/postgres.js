...
pool.query('select * from pg_stat_database WHERE datname = $1', [configPrivate.postgres.database], (err, res) => {
	if (err) {
		log.error('⚙️  Postgres database error: %s', err.message)
		throw err
	}

	log.info('⚙️  Connected to Postgres database! Base have %s connections', res.rows[0].numbackends)
})
