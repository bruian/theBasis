	...
	describe.only('#Controllers tests', function() {
		describe('User controller test', function() {
			...
			it('Attempt to read users object', function() {
				UsersController.update({ email: 'user1@maus.club' }, { username: 'maus' }, true)
					.then(result => {
		...
		describe('#MongoCache test cache and mongodb equals', function() {
			...
			it('Should set many values in the Cache and async in the BlacklistTokenModel', function(done) {
				...
				for (let index = 0; index < 10; index++) {
					...
					should.not.exist(
						mongoCache.set(key, { value: dateValue }, err => {
							if (!err) done();
						}),
					);
				}
			...
			it('Data Integrity Check', function() {
				...
				return BlacklistTokenModel.find({ key: { $in: keyArray } }).then(tokens => {
					should.exist(tokens);
			...
			after(function() {
				return BlacklistTokenModel.remove({ key: { $in: keyArray } }).then(() => {
